...
impl Parser {
    ...
    fn is_eof(&self) -> bool { matches!(self.peek().kind, TokenKind::Eof) }
    ...
    fn parse_function(&mut self) -> Result<Function> {
        ...
        if !matches!(self.peek().kind, TokenKind::RParen) {
            ...
            while matches!(self.peek().kind, TokenKind::Comma) { self.advance(); params.push(self.parse_param()?); }
        }
    ...
    fn parse_block_until_else_or_end(&mut self) -> Result<Vec<Stmt>> {
        let mut stmts = Vec::new();
        while !matches!(self.peek().kind, TokenKind::Else | TokenKind::End | TokenKind::Eof) {
            stmts.push(self.parse_stmt()?);
        }
        Ok(stmts)
    }
    ...
    fn parse_stmt(&mut self) -> Result<Stmt> {
        match self.peek().kind.clone() {
            TokenKind::Let => {
                ...
                let name = self.consume_ident()?;
                let mut ty = None;
                if matches!(self.peek().kind, TokenKind::Colon) { self.advance(); ty = Some(self.parse_type_name()?); }
            ...
            TokenKind::Ident(_) => {
                ...
                if let TokenKind::Ident(name) = self.peek().kind.clone() {
                    let is_assign = matches!(self.tokens.get(self.pos + 1).map(|t| &t.kind), Some(TokenKind::Equal));
                    if is_assign {
                        self.advance();
                        self.expect(TokenKind::Equal)?;
                        let expr = self.parse_expr()?;
    ...
    pub fn parse_expr(&mut self) -> Result<Expr> { self.parse_logical_or() }
    ...
    fn parse_equality(&mut self) -> Result<Expr> {
        ...
        loop {
            match self.peek().kind.clone() {
                TokenKind::EqEq => { self.advance(); let right = self.parse_comparison()?; left = Expr::Eq(Box::new(left), Box::new(right)); }
                TokenKind::NotEq => { self.advance(); let right = self.parse_comparison()?; left = Expr::Ne(Box::new(left), Box::new(right)); }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_comparison(&mut self) -> Result<Expr> {
        ...
        loop {
            match self.peek().kind.clone() {
                TokenKind::Less => { self.advance(); let right = self.parse_term()?; left = Expr::Lt(Box::new(left), Box::new(right)); }
                TokenKind::LessEq => { self.advance(); let right = self.parse_term()?; left = Expr::Le(Box::new(left), Box::new(right)); }
                TokenKind::Greater => { self.advance(); let right = self.parse_term()?; left = Expr::Gt(Box::new(left), Box::new(right)); }
                TokenKind::GreaterEq => { self.advance(); let right = self.parse_term()?; left = Expr::Ge(Box::new(left), Box::new(right)); }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_term(&mut self) -> Result<Expr> {
        ...
        loop {
            match self.peek().kind.clone() {
                TokenKind::Plus => { self.advance(); let right = self.parse_factor()?; left = Expr::BinaryAdd(Box::new(left), Box::new(right)); }
                TokenKind::Minus => { self.advance(); let right = self.parse_factor()?; left = Expr::BinarySub(Box::new(left), Box::new(right)); }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_factor(&mut self) -> Result<Expr> {
        ...
        loop {
            match self.peek().kind.clone() {
                TokenKind::Star => { self.advance(); let right = self.parse_unary()?; left = Expr::BinaryMul(Box::new(left), Box::new(right)); }
                TokenKind::Slash => { self.advance(); let right = self.parse_unary()?; left = Expr::BinaryDiv(Box::new(left), Box::new(right)); }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_unary(&mut self) -> Result<Expr> {
        match self.peek().kind.clone() {
            TokenKind::Bang => { self.advance(); let e = self.parse_unary()?; Ok(Expr::LogicalNot(Box::new(e))) }
    ...
    fn parse_primary(&mut self) -> Result<Expr> {
        ...
        let mut node = match tk.kind {
            ...
            TokenKind::False => { self.advance(); Ok(Expr::LiteralBool(false)) }
            TokenKind::Ident(name) => {
                if matches!(self.tokens.get(self.pos + 1).map(|t| &t.kind), Some(TokenKind::LParen)) {
                    ...
                    if !matches!(self.peek().kind, TokenKind::RParen) {
                        ...
                        while matches!(self.peek().kind, TokenKind::Comma) { self.advance(); args.push(self.parse_expr()?); }
                    }
                    self.expect(TokenKind::RParen)?;
    ...
    fn expect(&mut self, kind: TokenKind) -> Result<()> {
        ...
        if std::mem::discriminant(&tk.kind) == std::mem::discriminant(&kind) {
            ...
            zirc_syntax::error::error_at(tk.line, tk.col, format!("Expected {:?}, found {:?}", kind, tk.kind))
        }
    }
}
