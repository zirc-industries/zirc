...
impl Lexer {
    ...
    fn peek_next(&self) -> Option<char> { self.src.get(self.pos + 1).copied() }
    ...
    fn read_number(&mut self) -> Result<Token> {
        ...
        let val: i64 = s.parse().map_err(|_| zirc_syntax::error::Error::with_span("Invalid number", start_line, start_col))?;
        Ok(Token { kind: TokenKind::Number(val), line: start_line, col: start_col })
    }
    ...
    fn read_string(&mut self) -> Result<Token> {
        ...
        while let Some(c) = self.advance() {
            match c {
                '"' => {
                    return Ok(Token { kind: TokenKind::String(s), line: start_line, col: start_col });
                }
                '\\' => {
                    if let Some(n) = self.advance() {
                        ...
                        return zirc_syntax::error::error_at(start_line, start_col, "Unterminated string");
                    }
                }
                other => s.push(other),
            }
    ...
    pub fn tokenize(&mut self) -> Result<Vec<Token>> {
        ...
        loop {
            ...
            let tk = match self.peek() {
                ...
                Some('!') => {
                    if self.peek_next() == Some('=') { self.advance(); self.advance(); Token { kind: TokenKind::NotEq, line, col } }
                    else { self.advance(); Token { kind: TokenKind::Bang, line, col } }
                }
                Some('<') => {
                    self.advance();
                    if self.peek() == Some('=') { self.advance(); Token { kind: TokenKind::LessEq, line, col } } else { Token { kind: TokenKind::Less, line, col } }
                }
                Some('>') => {
                    self.advance();
                    if self.peek() == Some('=') { self.advance(); Token { kind: TokenKind::GreaterEq, line, col } } else { Token { kind: TokenKind::Greater, line, col } }
                }
                Some('+') => { self.advance(); self.make_token(TokenKind::Plus) }
                Some('-') => { self.advance(); self.make_token(TokenKind::Minus) }
                Some('*') => { self.advance(); self.make_token(TokenKind::Star) }
                Some('/') => { self.advance(); self.make_token(TokenKind::Slash) }
                Some('&') => {
                    if self.peek_next() == Some('&') { self.advance(); self.advance(); Token { kind: TokenKind::AndAnd, line, col } }
                    else { return zirc_syntax::error::error_at(line, col, "Unexpected '&' (did you mean '&&'?)"); }
                }
                Some('|') => {
                    if self.peek_next() == Some('|') { self.advance(); self.advance(); Token { kind: TokenKind::OrOr, line, col } }
                    else { return zirc_syntax::error::error_at(line, col, "Unexpected '|' (did you mean '||'?)"); }
                }
                Some('"') => { self.advance(); self.read_string()? }
                Some(c) if c.is_ascii_digit() => { self.read_number()? }
                Some(c) if c.is_ascii_alphabetic() || c == '_' => { self.read_ident_or_keyword() }
                Some(other) => { return zirc_syntax::error::error_at(line, col, format!("Unexpected character '{}'", other)); }
            };
            tokens.push(tk);
        }
        Ok(tokens)
